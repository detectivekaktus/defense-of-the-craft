package net.detectivekaktus.sound.gui;

import net.minecraft.sounds.SoundEvent;

import net.detectivekaktus.sound.DotcSounds;

public class DotcGuiSounds {
    public static final SoundEvent UI_STEAM_CAMERA = DotcSounds.register("ui_steam_camera");
    public static final SoundEvent UI_BUTTON_PRESS = DotcSounds.register("ui_button_press");
    public static final SoundEvent UI_BUTTON_CONFIRM = DotcSounds.register("ui_button_confirm");
    public static final SoundEvent UI_CHANGE_TAB = DotcSounds.register("ui_change_tab");
    public static final SoundEvent UI_EVADED = DotcSounds.register("ui_evaded");
    public static final SoundEvent UI_NOT_ENOUGH_MANA = DotcSounds.register("ui_not_enough_mana");
    public static final SoundEvent UI_COOLDOWN = DotcSounds.register("ui_cooldown");
    public static final SoundEvent UI_GENERAL_DENY = DotcSounds.register("ui_general_deny");
    public static final SoundEvent UI_DEBUFF = DotcSounds.register("ui_debuff");
    public static final SoundEvent UI_IMMUNE = DotcSounds.register("ui_immune");
    public static final SoundEvent UI_SILENCE = DotcSounds.register("ui_silence");
    public static final SoundEvent UI_RECEIVED_MESSAGE = DotcSounds.register("ui_received_message");

    public static void initialize() { }
}
